...
class ProductQueryAgent(BaseBusinessAgent):
    ...
    async def run(self, question: str, ctx: AgentRunContext) -> str:
        ...
        supplier_data: Any = await run_configured_skills(skill_keys=skill_keys, question=question, ctx=ctx)
        ...
        try:
            ...
            logger.info(
                "[STRUCT_DEBUG][product_query] llm_done elapsed=%.3fs reply_len=%d",
            ...
            try:
                ...
                logger.info(
                    "[STRUCT_DEBUG][product_query] structured_ready elapsed=%.3fs count=%d",
