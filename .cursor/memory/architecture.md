...
- **Orchestrator（任务编排/多智能体调度）**
  - 智能体选择、工具调用编排（数据查询、检索、抽取、生成）、超时与重试、并行化、降级与缓存策略。
  - 输出统一为“结构化响应 + 引用 + 风险提示 + 审计信息”。
...
### 前端（H5）对内的核心接口（示例边界）
- `POST /api/chat`：多轮对话入口（支持会话 id、上下文、产品选择、客户画像）；根据意图路由到 FAQ / RAG / 产品解读 / 产品列表查询 / 洞察等智能体；响应可含 `suggestedQuestions[]`（猜你想问）。
