...
**一句话描述**：通过 H5 交互 + 意图识别 + 记忆模块 + 多智能体协同，在合规约束下为银行客户经理提供实时、可追溯的产品要素解读、产品匹配与对比、研报/政策摘要以及周报月报等专业内容生成能力，显著提升销售沟通效率与客户体验。
...
### 4.2 意图识别与任务路由
- **意图分类**
  - 识别：产品要素解读、产品对比、产品检索推荐、研报摘要、政策解读、报告生成、闲聊/无关等。
- **槽位抽取**
  - 抽取：产品名称/代码、产品类型、时间范围、对比维度、客户画像（期限/风险/流动性/收益目标等）。
- **路由策略**
  - 根据意图与槽位选择智能体组合（如：RAG + 产品要素抽取 + 合规校验）。
...
```mermaid
flowchart LR
  A[客户经理在H5输入问题/选择产品] --> B[意图识别&槽位抽取]
  B --> C[任务编排/路由到相关智能体]
  C --> D[数据访问层获取产品要素/行情等]
  C --> E[RAG检索研报/条款/政策证据]
  D --> F[答案生成与结构化输出]
